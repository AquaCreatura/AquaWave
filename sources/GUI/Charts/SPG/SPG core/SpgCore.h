...
public:
	...
	void MulBoundsByKoeff	(const double mul_koeff);
    ...
    //Is Used to get pixmap from our kBaseSpg
    QPixmap&         GetRelevantPixmap(const ChartScaleInfo& scale_info);
    spg_data const & GetSpectrogramInfo() const;
protected:
	void SetDataToRow(const std::vector<float>& passed_data, Limits<size_t> row_id, size_t column_idx, spg_holder& holder_to_fill);
private:
    spg_data    spg_;
